...
export const scoreMapFn = (score: Score, i: number, strokeWidth: number, offset: number, height: number) => {
  return { ...score, x2: (strokeWidth * (i + 1) + (i * offset)) + strokeWidth / 2, y1: height / 2, y2: score.tally >= 0 ? (height / 2 - score.tally) : height / 2 + Math.abs(score.tally) }
};
