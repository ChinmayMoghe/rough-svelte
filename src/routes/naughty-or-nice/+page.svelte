...
export let data:{scores:Array<Score>,svg:{width:number,height:number,viewBox:Array<number>,pathData:string,strokeWidth:number,offset:number}};
...
function plotGraph(scores:Array<Score>) {
  return scores.map((score,i)=>scoreMapFn(score,i,data.svg.strokeWidth,data.svg.offset,data.svg.height));    
}
...
<div class="grid">
<div>
   <button on:click={()=>{sortHeader('name')}}>Name {sortProps.sortBy === 'name' ? sortProps.sortDir === 'asc' ? '⬆️' : '⬇️' : ' '}</button>
   <button on:click={()=>{sortHeader('tally')}}>Tally {sortProps.sortBy === 'tally' ? sortProps.sortDir === 'asc' ? '⬆️' : '⬇️' : ' '}</button>  
{#if hoverScore}
...
<svg width={data.svg.width} height={data.svg.height} viewBox={data.svg.viewBox.join(' ')}>
    ...
    <line x1={0} y1={data.svg.height / 2} x2={data.svg.width} y2={data.svg.height / 2} stroke-width='2' stroke='transparent'/>
    {#each scores as score}
      <circle tabindex="0" role="button" cx={score.x2} cy={score.y2} r={data.svg.strokeWidth} fill="red" on:mouseover={()=>hoverIn(score)} on:mouseout={()=>hoverOut()} on:focus={()=>hoverIn(score)} on:blur={()=>hoverOut()}/>
    {/each}
</svg>
